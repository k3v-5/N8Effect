...
public:
    ...
    void process(const ExecutionPlan& plan, const ProcessContext& mainContext, PreallocatedBuffer& finalOutput) {
        ...
        if (multithreadingEnabled_ && totalSteps > 2) {
            ...
            // Despacho secuencial monohilo
            for (size_t i = 0; i < totalSteps; ++i) {
                const auto& step = steps[i];
                PreallocatedBuffer* outBuf = stepOutputBuffers[i];
                if (outBuf == nullptr) break;
        ...
        if (probeVisualizer_ != nullptr) {
            const NodeId targetProbeId = probeNodeId_.load(std::memory_order_relaxed);
            for (size_t i = 0; i < totalSteps; ++i) {
                if (steps[i].nodeId == targetProbeId && stepOutputBuffers[i] != nullptr) {
                    const auto* pBuf = stepOutputBuffers[i];
                    probeVisualizer_->writeBlock(pBuf->getReadPointer(0),
                                                 (pBuf->getNumChannels() > 1) ? pBuf->getReadPointer(1) : pBuf->getReadPointer(0),
                                                 numSamples);
                    break;
                }
            }
